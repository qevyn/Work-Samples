...
def extract_pages_from_excel(file_path):
...
def transform_pages_excel(file_path, brand, indication, date_range):
    ...
    channel_columns = [col for col in df.columns if col != "PAGE"]

    # Melt the dataframe into long format
    df_melted = pd.melt(df, id_vars=["PAGE"], value_vars=channel_columns, var_name="CHANNEL_METRIC", value_name="VALUE")
    ...
    df_final = df_melted.pivot_table(index=['PAGE', 'CHANNEL'], columns='METRIC', values='VALUE').reset_index()
    ...
    # Reorder columns
    desired_order = ['PAGE', 'CHANNEL', 'VISITS', 'BOUNCES']
    ...
    numeric_columns = ['VISITS', 'BOUNCES']
    df_final[numeric_columns] = df_final[numeric_columns].fillna(0)

    # Save final output to an Excel file
    output_path = f"{brand}_{indication}_{date_range}_PAGES.xlsx"
    with pd.ExcelWriter(output_path, engine='openpyxl', mode='w') as writer:
        df_final.to_excel(writer, sheet_name='BMS Adobe Pages Website', index=False)
...
if __name__ == "__main__":
    ...
    # Step 2: Extract and transform the PAGES table
    try:
        extracted_pages_table_path = extract_pages_from_excel(input_excel)
        temp_files.append(extracted_pages_table_path)  # Keep track of temporary file
        transform_pages_excel(extracted_pages_table_path, brand, indication, date_range)
    except ValueError as e:
        print(f"Error processing PAGES table: {e}")

...
def extract_targeting_from_excel(file_path):
...
def transform_targeting_excel(file_path, brand, indication, date_range):
    ...
    df.rename(columns={df.columns[0]: "CHANNEL", df.columns[1]: "TARGET_METHODOLOGY"}, inplace=True)
    ...
    df_melted = pd.melt(df, id_vars=["CHANNEL", "TARGET_METHODOLOGY"], value_vars=date_columns, var_name="ACTIVITY_MONTH", value_name="VALUE")
    ...
    df_final = df_melted.pivot(index=["ACTIVITY_MONTH", "CHANNEL", "TARGET_METHODOLOGY"], columns="METRIC", values="VALUE").reset_index()

    # Rename columns to match the desired output
    df_final.columns.name = None
    df_final = df_final.rename(columns={
        "TARGET_METHODOLOGY": "TARGET METHODOLOGY", 
        'ACTIVITY_MONTH': 'ACTIVITY MONTH',
        'Visits': 'VISITS'
    })

    # Reorder columns
    desired_order = ["CHANNEL", "TARGET METHODOLOGY", 'ACTIVITY MONTH', 'VISITS']
    df_final = df_final[desired_order]

    # Sort by LAST_TOUCH_CHANNEL then ACTIVITY_MONTH
    df_final = df_final.sort_values(by=["CHANNEL", "TARGET METHODOLOGY", 'ACTIVITY MONTH'])

    # Fill blanks as zeroes
    numeric_columns = ['VISITS']
    ...
    output_path = f"{brand}_{indication}_{date_range}_TARGETING.xlsx"
    with pd.ExcelWriter(output_path, engine='openpyxl', mode='w') as writer:
        df_final.to_excel(writer, sheet_name='BMS Adobe Targeting Website', index=False)
...
if __name__ == "__main__":
    ...
    # Step 2: Extract and transform the TARGETING table
    try:
        extracted_targeting_table_path = extract_targeting_from_excel(input_excel)
        temp_files.append(extracted_targeting_table_path)  # Keep track of temporary file
        transform_targeting_excel(extracted_targeting_table_path, brand, indication, date_range)
    except ValueError as e:

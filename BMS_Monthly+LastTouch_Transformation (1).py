...
def transform_lasttouch_excel(file_path, brand, indication, date_range):
    ...
    df_final = df_final.rename(columns={
        ...
        'ACTIVITY_MONTH': 'ACTIVITY DATE',
        'LAST_TOUCH_CHANNEL': 'LAST TOUCH CHANNEL'
    })

    # Reorder columns
    desired_order = ['ACTIVITY DATE', 'LAST TOUCH CHANNEL', 'UNIQUE VISITORS', 'VISITS', 'UNBOUNCED VISIT', 'BOUNCE RATE', 'PAGE VIEWS PER VISITS', 'AVERAGE TIME ON SITE']
    df_final = df_final[desired_order]

    # Sort by LAST_TOUCH_CHANNEL then ACTIVITY_MONTH
    df_final = df_final.sort_values(by=['LAST TOUCH CHANNEL', 'ACTIVITY DATE'])
    ...
    df_final['ACTIVITY DATE'] = pd.to_datetime(df_final['ACTIVITY DATE'])

    # Add BRAND and INDICATION columns
    df_final['BRAND'] = brand
    df_final['INDICATION'] = indication

    df_final['IQVIA AQ INDEX'] = 0
    df_final['CROSSIX INDEX'] = 0
